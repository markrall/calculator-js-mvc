...
class CalculatorView {
    constructor() {
        ...
        __classPrivateFieldSet(this, _parentElement, document.getElementById('root'));
        this.operator = '';
        this.input = '0';
        this.operand = 0;
        this.display = null;
    ...
    clickEventHandler(element, handler) {
        ...
        if (element.classList.contains(keyType.operandKey)) {
            ...
            if (keyValue === '.') {
                if (this.input.includes(keyValue))
                    return;
                if (this.input === '') {
                    this.input = '0.';
                }
            ...
            if (isDevelopment)
                console.log('CalculatorView.:clickEventHandler:thisInput: ', this.input);
    ...
    addHandlerClick(handler) {
        __classPrivateFieldGet(this, _parentElement).addEventListener('click', e => {
            const element = e.target;
            // Ignore click events on non-key elements
            if (!element.classList.contains('calc__key'))
                return;
    ...
    render() {
        __classPrivateFieldGet(this, _parentElement).innerHTML = this.generateMarkup();
